...
template <typename ContainerType, typename timeOpFunctType, typename enforceValueFunctType, typename errorFunctType, typename outputFunctType, typename timeStepVetoerType> void dormandPrince54
 (ContainerType&                                timeContainer,      // input/output
  const typename ContainerType::ArgType         finalTime,          // end time of propagation
  const typename ContainerType::ArgType         denseTimeStep,      // step for dense grid output
  timeOpFunctType                               timeOp,             // operator that gives the derivative
  ...
  outputFunctType                               outputFunct,         // Defines some output operations
  // It must be in the form >     outputFunct(ContainerType::ArgType currentTime, ContainerType::ValType currentY)
  timeStepVetoerType                            timeStepVetoer       // Defines a function that vets the time step
  // It must be in the form >     timeStepVetoer(std::vector<ContainerType::ArgType> timesToTest, ContainerType::ArgType denseStep) -> ContainerType::ArgType maxAllowedStep
    ...
    std::vector<typename ContainerType::ArgType> timesToTest{currentTime, currentTime + c2 * timeStepSize, currentTime + c3 * timeStepSize, currentTime + c4 * timeStepSize, currentTime + c5 * timeStepSize, currentTime + timeStepSize};
    
    timeStepSize = std::min(timeStepSize, timeStepVetoer(timesToTest, denseTimeStep));
...
//            std::cout << " ACCEPTED <<< ";
            ...
            // Vetting of the timestep with the user-provided function
            std::vector<typename ContainerType::ArgType> timesToTest{currentTime, currentTime + c2 * timeStepSize, currentTime + c3 * timeStepSize, currentTime + c4 * timeStepSize, currentTime + c5 * timeStepSize, currentTime + timeStepSize};
